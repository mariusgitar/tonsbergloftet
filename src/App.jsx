...
// ── Fargepalett – Tønsberg kommune ────────────────────────────────────────────
const NAVY        = "#013952";
const NAVY_MID    = "#0a2e40";
const NAVY_BG     = "#0d2d35";
const TEAL        = "#013952";   // brukes som primær aksent (samme som NAVY)
const TEAL_DARK   = "#012840";
const TEAL_LIGHT  = "#e8f0f3";
const TEAL_BG     = NAVY_BG;
const GREEN       = "#c5daaf";
const GREEN_DARK  = "#00714e";
const CORAL       = "#e69d7d";
const YELLOW      = "#f1be48";
const BLUE_LIGHT  = "#91cef2";
const OFF_WHITE   = "#f7f4ef";
const INK         = "#1a1a1a";
const WARM_GRAY   = "#6b6560";

// LLP fase-farger (beholdes som de er – de er ikke kommunens profil, men LLP-brand)
const LLP_LAERE  = "#91cef2";   // lyseblå – hjerte
const LLP_LAGE   = "#f1be48";   // gul – diamant
const LLP_PROVE  = "#e69d7d";   // korall – sirkel
...
const llpChapters = [
  {
    ...
    accent: LLP_LAERE, pattern: "circles",
  ...
  },
  {
    ...
    accent: LLP_LAGE, pattern: "grid",
    punkter: ["Lag mange idéer raskt – ikke én perfekt", "Bygg enkle prototyper og skisser", "Konkretiser slik at andre kan forholde seg til det"],
  },
  {
    ...
    accent: LLP_PROVE, pattern: "dots",
    punkter: ["Planlegg testen godt – gi en oppgave brukeren skal løse", "Lytt aktivt og still gode spørsmål", "Samle ærlige tilbakemeldinger", "Ta med lærdommene inn i neste runde"],
  },
  { type: "loop", accent: LLP_LAERE, pattern: "circles" },
  { type: "hoppinn-lage", accent: LLP_LAGE, pattern: "grid" },
  { type: "hoppinn-prove", accent: LLP_PROVE, pattern: "dots" },
];
...
function FaseBadge({ fase }) {
  var col = fase === "Lære" ? LLP_LAERE : fase === "Lage" ? LLP_LAGE : LLP_PROVE;
  return <span style={{ fontSize: 10, fontWeight: 700, color: col, background: col+"22", border: "1px solid "+col+"44", padding: "3px 10px", borderRadius: 20, letterSpacing: 0.5, textTransform: "uppercase" }}>{fase}</span>;
}
...
function OppfolgingsBoks({ answers, onSvar }) {
  ...
  if (!answers[2]) return null;
  return (
    <div style={{ background: "white", borderRadius: 12, padding: "28px 32px", border: "1px solid rgba(1,57,82,0.15)", maxWidth: 540, boxShadow: "0 4px 24px rgba(1,57,82,0.08)" }}>
      <p style={{ color: NAVY, fontWeight: 700, fontSize: 11, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10 }}>Oppfølging</p>
      <p style={{ fontSize: 16, color: INK, fontWeight: 600, marginBottom: 8 }}>
        Du forventet: <span style={{ fontFamily: "'Caveat', cursive", fontSize: 20, color: NAVY }}>"{answers[2]}"</span>
      </p>
      {!sendt ? (
        <div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {["Ja, absolutt!", "Delvis", "Ikke helt", "Nei, men interessant"].map(function(alt) {
              return <button key={alt} onClick={function() { setSvar(alt); setSendt(true); onSvar && onSvar(alt); }} style={{ background: OFF_WHITE, border: "1px solid rgba(0,0,0,0.1)", borderRadius: 20, padding: "7px 14px", fontSize: 13, color: INK, cursor: "pointer", fontFamily: "'Barlow', sans-serif" }}>{alt}</button>;
            })}
          </div>
        </div>
      ) : (
        <div>
          <p style={{ fontSize: 16, color: INK }}>Du svarte: <strong style={{ color: NAVY }}>{svar}</strong></p>
...
function LoopChapter() {
  // Karusell med sirkelbilder + label + pil, replikert 4x for sømløs loop
  var faser = [
    { label: "Lære", img: "/Lære sirkel.png", col: LLP_LAERE },
    { label: "Lage", img: "/Lage sirkel.png", col: LLP_LAGE },
    { label: "Prøve", img: "/Prøve sirkel.png", col: LLP_PROVE },
  ];
  var rep = faser.concat(faser).concat(faser).concat(faser);
  return (
    <div style={{ padding: BAR_HEIGHT+"px 8vw 48px", overflowY: "auto", width: "100%", maxWidth: 860, margin: "0 auto", display: "flex", flexDirection: "column", justifyContent: "center", minHeight: "100vh" }}>
      <p style={{ color: LLP_LAERE, fontWeight: 700, fontSize: 10, letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>Slik fungerer det i praksis</p>
      <h2 style={{ fontSize: "clamp(28px,6vw,44px)", fontWeight: 800, color: "white", lineHeight: 1.15, marginBottom: 12 }}>LLP er en måte å tenke på.</h2>
      ...
      <style>{`
        ...
        .loop-track{display:flex;animation:scrollLeft 12s linear infinite;width:max-content;}
        .loop-track:hover{animation-play-state:paused;}
      `}</style>
      <div style={{ overflow: "hidden", marginLeft: -48, marginRight: -48, padding: "8px 0 20px", marginBottom: 24 }}>
...
function HoppInnLage() {
  return (
    <div style={{ padding: BAR_HEIGHT+"px 8vw 48px", overflowY: "auto", width: "100%", maxWidth: 860, margin: "0 auto", display: "flex", flexDirection: "column", justifyContent: "center", minHeight: "100vh" }}>
      <p style={{ color: LLP_LAGE, fontWeight: 700, fontSize: 10, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>En siste ting</p>
      <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 24 }}>
        <div style={{ background: LLP_LAGE, borderRadius: 4, padding: "4px 12px", fontSize: 11, fontWeight: 700, color: NAVY_BG }}>Lage</div>
      ...
      <div style={{ display: "flex", gap: 32, alignItems: "center" }}>
        {/* Venstre: tekst */}
        <div style={{ flex: 1 }}>
          <h2 style={{ fontSize: "clamp(24px,5vw,36px)", fontWeight: 800, color: "white", lineHeight: 1.15, marginBottom: 12 }}>Har du en skikkelig god idé?</h2>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", lineHeight: 1.75, marginBottom: 16 }}>Da kan du godt begynne rett i Lage-fasen. Bygg noe konkret, få det ut i hendene på folk – og la tilbakemeldingene ta deg tilbake til Lære etterpå.</p>
          <p style={{ fontFamily: "'Caveat', cursive", fontSize: 20, color: LLP_LAGE, marginBottom: 20 }}>"Ikke la mangel på innsikt stoppe en god idé – bare vær åpen for å ta feil."</p>
          <div style={{ background: "rgba(241,190,72,0.1)", border: "1px solid rgba(241,190,72,0.25)", borderRadius: 8, padding: "14px 16px" }}>
...
function HoppInnProve() {
  return (
    <div style={{ padding: BAR_HEIGHT+"px 8vw 48px", overflowY: "auto", width: "100%", maxWidth: 860, margin: "0 auto", display: "flex", flexDirection: "column", justifyContent: "center", minHeight: "100vh" }}>
      <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 24 }}>
        <div style={{ background: LLP_PROVE, borderRadius: 4, padding: "4px 12px", fontSize: 11, fontWeight: 700, color: "white" }}>Prøve</div>
        <div style={{ color: "rgba(255,255,255,0.2)", fontSize: 12 }}>→</div>
        <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 4, padding: "4px 12px", fontSize: 11, color: "rgba(255,255,255,0.25)" }}>Lære</div>
        <div style={{ color: "rgba(255,255,255,0.2)", fontSize: 12 }}>→</div>
      ...
      <div style={{ display: "flex", gap: 32, alignItems: "center" }}>
        {/* Venstre: tekst */}
        <div style={{ flex: 1 }}>
          <h2 style={{ fontSize: "clamp(24px,5vw,36px)", fontWeight: 800, color: "white", lineHeight: 1.15, marginBottom: 12 }}>Vil du forbedre noe som allerede finnes?</h2>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", lineHeight: 1.75, marginBottom: 16 }}>Da kan du hoppe rett til Prøve. Test det som allerede finnes, samle ærlige tilbakemeldinger – og la det ta deg tilbake til Lære eller Lage når du vet hva som må forbedres.</p>
          <p style={{ fontFamily: "'Caveat', cursive", fontSize: 20, color: LLP_PROVE, marginBottom: 20 }}>"Den eneste måten å vite er å teste."</p>
          <div style={{ background: "rgba(230,157,125,0.1)", border: "1px solid rgba(230,157,125,0.25)", borderRadius: 8, padding: "14px 16px" }}>
          ...
          {llpChapters.map(function(ch, i) {
            ...
            return (
              <div key={i} onClick={function() { onJump(i); }} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, cursor: "pointer" }}>
                <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1, color: isActive?"white":"transparent", fontFamily: "'Barlow', sans-serif", textTransform: "uppercase", whiteSpace: "nowrap", transition: "color 0.3s ease", height: 12, lineHeight: "12px" }}>{label}</div>
    ...
    <div>
      ...
      <div style={{ background: OFF_WHITE, padding: "48px 24px 56px" }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          <p style={{ color: NAVY, fontWeight: 700, fontSize: 10, letterSpacing: 2.5, textTransform: "uppercase", marginBottom: 10 }}>Metoden vår</p>
          <h2 style={{ fontSize: "clamp(26px,6vw,36px)", fontWeight: 800, color: INK, lineHeight: 1.15, marginBottom: 12 }}>Lære. Lage. Prøve.<br />I den rekkefølgen.</h2>
          ...
          {/* LLP-kortene – matcher screenshotten */}
          <div style={{ display: "flex", gap: 16, marginTop: 36, flexWrap: "wrap" }}>
            {[
              { title: "Lære", shape: "heart", color: LLP_LAERE, desc: "Finn ut hva som er viktig" },
              { title: "Lage", shape: "diamond", color: LLP_LAGE, desc: "Lag noe konkret" },
              { title: "Prøve", shape: "circle", color: LLP_PROVE, desc: "Få feedback" },
            ].map(function(k) {
              return (
                <div key={k.title} style={{ flex: "1 1 140px", background: "white", borderRadius: 14, padding: "22px 18px 18px", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 2px 10px rgba(0,0,0,0.04)", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                  <p style={{ fontSize: 20, fontWeight: 800, color: INK, marginBottom: 16 }}>{k.title}</p>
                  <div style={{ marginBottom: 14, height: 60, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {k.shape === "heart" && (
                      <svg width="54" height="50" viewBox="0 0 54 50"><path d="M27 46C27 46 4 32 4 17a11 11 0 0 1 23-2.2A11 11 0 0 1 50 17c0 15-23 29-23 29z" fill={k.color}/></svg>
                    )}
                    {k.shape === "diamond" && (
                      <svg width="62" height="46" viewBox="0 0 62 46"><polygon points="31,3 59,23 31,43 3,23" fill={k.color}/><line x1="31" y1="3" x2="31" y2="43" stroke="white" stroke-width="1.5" opacity="0.5"/></svg>
                    )}
                    {k.shape === "circle" && (
                      <svg width="52" height="52" viewBox="0 0 52 52"><circle cx="26" cy="26" r="22" fill={k.color}/></svg>
                    )}
                  </div>
                  <div style={{ display: "flex", gap: 4, marginBottom: 10 }}>
                    {[0,1,2].map(function(d) { return <div key={d} style={{ width: 5, height: 5, borderRadius: "50%", background: "#ccc" }} />; })}
                  </div>
                  <p style={{ fontSize: 12, color: WARM_GRAY, margin: 0, lineHeight: 1.5 }}>{k.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      <div ref={sectionRef} style={{ height: (llpChapters.length * 160) + "vh", position: "relative" }}>
        <div style={{ position: "sticky", top: 0, height: "100vh", overflow: "hidden", display: "flex", flexDirection: "column" }}>
          <div style={{ position: "absolute", inset: 0, background: NAVY_BG }}>
            <div style={{ position: "absolute", inset: "-20%" }}><Pattern type={chapter.pattern||"circles"} color={chapter.accent||NAVY} /></div>
          </div>
            ...
            {llpChapters.map(function(ch, i) {
              ...
              return (
                <div key={i} style={{ width: "100vw", height: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", flexShrink: 0, opacity: isActive?1:Math.max(0,1-Math.abs(dist)*0.85), transform: "scale("+(isActive?1:0.95)+") translateX("+(dist*6)+"px)", transition: "opacity 0.5s ease, transform 0.5s ease" }}>
                  {ch.type === "phase" && (
                    <div style={{ padding: BAR_HEIGHT+"px 8vw 48px", overflowY: "auto", width: "100%", maxWidth: 860 }}>
                      {/* To-kolonne: tekst venstre, ikon høyre */}
                      <div style={{ display: "flex", gap: 36, alignItems: "center" }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <h2 style={{ fontSize: "clamp(32px,7vw,52px)", fontWeight: 800, color: "white", lineHeight: 1.0, marginBottom: 4 }}>{ch.phaseLabel}</h2>
                          <div style={{ width: 24, height: 2, background: ch.accent, marginBottom: 8 }} />
                          <p style={{ fontFamily: "'Caveat', cursive", fontSize: "clamp(16px,3vw,20px)", color: ch.accent, lineHeight: 1.3, marginBottom: 8 }}>"{ch.laerdom}"</p>
                          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", lineHeight: 1.65, marginBottom: 12, maxWidth: 380 }}>{ch.desc}</p>
          ...
          <div style={{ position: "absolute", bottom: 14, left: 0, right: 0, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 20px", zIndex: 20 }}>
            ...
            <button onClick={skipToEnd} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.25)", fontSize: 10, cursor: "pointer", fontFamily: "'Barlow', sans-serif", letterSpacing: 1, textTransform: "uppercase" }}>Hopp over</button>
          </div>
        </div>
      </div>
  ...
  return (
    <section style={{ padding: "48px 24px" }}>
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        {erTonsberg && (
          <div style={{ background: "#e8f2f4", border: "1px solid "+NAVY+"22", borderRadius: 8, padding: "12px 16px", marginBottom: 20, display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 18 }}>👋</span>
            <p style={{ fontSize: 14, color: NAVY, margin: 0, fontWeight: 600 }}>Hei, kollega! Godt å se deg her.</p>
          </div>
        )}
        <p style={{ color: NAVY, fontWeight: 700, fontSize: 10, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10 }}>Om TønsbergLØFTET</p>
        <h2 style={{ fontSize: "clamp(22px,5vw,28px)", fontWeight: 800, color: INK, marginBottom: 16, lineHeight: 1.2 }}>Vi støtter kommunen i å jobbe på nye måter.</h2>
        ...
        {/* Prinsipper – farget topp-stripe per kort */}
            ...
            return (
              <div key={card.label} style={{ background: "white", borderRadius: 10, overflow: "hidden", flex: "1 1 130px", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
                ...
                <div style={{ padding: "12px 14px" }}>
                  <p style={{ fontWeight: 700, color: INK, marginBottom: 2, fontSize: 13 }}>{card.label}</p>
                  <p style={{ color: WARM_GRAY, fontSize: 12, margin: 0 }}>{card.desc}</p>
                </div>
              </div>
            );
          })}
        ...
        {/* Slik kan vi hjelpe */}
        <p style={{ color: NAVY, fontWeight: 700, fontSize: 10, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10 }}>Slik kan vi hjelpe</p>
              ...
              <div key={t.title} style={{ background: "white", borderRadius: 10, padding: "14px 16px", border: "1px solid rgba(0,0,0,0.06)", display: "flex", gap: 14, alignItems: "flex-start" }}>
                ...
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                    <p style={{ fontWeight: 700, color: INK, fontSize: 14, margin: 0 }}>{t.title}</p>
                    <span style={{ background: "#e8f2f4", color: NAVY, fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 20, letterSpacing: 0.5, textTransform: "uppercase" }}>{t.tag}</span>
                  </div>
...
function SeksjonProsjekter() {
  ...
  return (
    <section style={{ padding: "56px 24px", background: "white" }}>
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <p style={{ color: NAVY, fontWeight: 700, fontSize: 10, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10 }}>Prosjektportefølje</p>
        <h2 style={{ fontSize: "clamp(22px,5vw,28px)", fontWeight: 800, color: INK, marginBottom: 8, lineHeight: 1.2 }}>Noen av prosjektene vi har jobbet med</h2>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {portefolje.map(function(p, i) {
            var erApen = aktivt === i;
            var statusFarge = p.status === "Pågår" ? GREEN_DARK : WARM_GRAY;
            return (
              <div key={p.title} onClick={function() { setAktivt(erApen ? null : i); }}
                style={{ background: erApen ? "#e8f2f4" : OFF_WHITE, borderRadius: 12, padding: "16px 18px", border: "1px solid "+(erApen ? NAVY+"22" : "rgba(0,0,0,0.06)"), cursor: "pointer", transition: "all 0.2s" }}>
                  ...
                  <div style={{ display: "flex", gap: 12, alignItems: "flex-start", flex: 1 }}>
                    ...
                    <div>
                      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 3 }}>
                        ...
                        <span style={{ fontSize: 10, fontWeight: 600, color: statusFarge, background: "rgba(0,0,0,0.05)", padding: "2px 7px", borderRadius: 20 }}>{p.status}</span>
                      </div>
                      <p style={{ fontSize: 12, color: WARM_GRAY, margin: 0 }}>{p.etat}</p>
                    </div>
                  </div>
                  <span style={{ fontSize: 16, color: NAVY, flexShrink: 0, marginTop: 2, transform: erApen ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>▾</span>
                </div>
                {erApen && (
                  <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid rgba(1,57,82,0.1)" }}>
                    <p style={{ fontSize: 13, color: WARM_GRAY, lineHeight: 1.7, marginBottom: 12 }}>{p.desc}</p>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
                      {p.faser.map(function(f) { return <FaseBadge key={f} fase={f} />; })}
                    </div>
                    <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                      <span style={{ fontSize: 14, flexShrink: 0 }}>📋</span>
                      <p style={{ fontSize: 12, color: NAVY, fontStyle: "italic", margin: 0, lineHeight: 1.5 }}>"{p.laerdom}"</p>
                    </div>
                  </div>
                )}
              </div>
  ...
  return (
    <section style={{ background: NAVY_BG, padding: "56px 0 0" }}>
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 24px" }}>
        ...
        <h2 style={{ fontSize: "clamp(22px,5vw,28px)", fontWeight: 800, color: "white", marginBottom: 12, lineHeight: 1.2 }}>Neste steg for TønsbergLØFTET</h2>
        ...
        {tabs.map(function(t) {
          ...
          return (
            <button key={t.id} onClick={function() { setAktivTab(t.id); }}
              style={{ background: "none", border: "none", cursor: "pointer", padding: "12px 8px", fontFamily: "'Barlow', sans-serif", fontSize: 12, fontWeight: er ? 700 : 500, color: er ? t.accent : "rgba(255,255,255,0.4)", borderBottom: er ? "2px solid "+t.accent : "2px solid transparent", whiteSpace: "nowrap", transition: "all 0.2s", marginBottom: -1, textAlign: "center", touchAction: "manipulation" }}>
              {t.emoji} {t.label}
            </button>
          );
        })}
      ...
      <div style={{ padding: "32px 24px 56px", maxWidth: 640, margin: "0 auto" }}>
              ...
              {[
                ...
                return (
                  <div key={k.label} style={{ background: "rgba(145,206,242,0.06)", border: "1px solid rgba(145,206,242,0.2)", borderRadius: 8, padding: "12px 16px" }}>
                    <p style={{ fontSize: 13, fontWeight: 700, color: LLP_LAERE, marginBottom: 4 }}>{k.label}</p>
        ...
        {aktivTab === "system" && (
          <div>
            <p style={{ fontSize: 22, fontFamily: "'Caveat', cursive", color: LLP_LAGE, marginBottom: 16 }}>"LLP er metoden. Men hva er riggen rundt?"</p>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", lineHeight: 1.8, marginBottom: 24 }}>Vi vil bygge et bedre system rundt innovasjonsarbeidet i Tønsberg. Ikke bare metoden, men strukturene, nettverkene og verktøyene som gjør det lettere for alle å jobbe på nye måter.</p>
            <img
              src="/Innovasjonssystem - kontinuerlig.png"
              alt="Innovasjonssystem – kontinuerlig"
...
function SeksjonProvLLP({ answers, isStatic }) {
  ...
  var [brukerhistorie, setBrukerhistorie] = useState(null);
  var [vinnerIde, setVinnerIde] = useState(null);
  if (isStatic) return null;
  var rolle = answers[0] || "annet";
  var kontekst = rolle === "tonsberg" ? "din avdeling" : rolle === "annenkommune" ? "din kommune" : "din organisasjon";
  var tabs = [
    { id: "laere", label: "Lære", accent: LLP_LAERE },
    { id: "lage", label: "Lage", accent: LLP_LAGE },
    { id: "prove", label: "Prøve", accent: LLP_PROVE },
  ];
  return (
    <section style={{ background: OFF_WHITE, padding: "56px 0 0" }}>
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "0 24px 24px" }}>
        <p style={{ color: NAVY, fontWeight: 700, fontSize: 10, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10 }}>Prøv selv</p>
        <h2 style={{ fontSize: "clamp(22px,5vw,28px)", fontWeight: 800, color: INK, marginBottom: 8, lineHeight: 1.2 }}>Prøv LLP i praksis</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", borderBottom: "1px solid rgba(0,0,0,0.08)", maxWidth: 600, margin: "0 auto" }}>
        {tabs.map(function(t) {
          ...
          return (
            <button key={t.id} onClick={function() { setAktivTab(t.id); }}
              style={{ background: "none", border: "none", cursor: "pointer", padding: "12px 8px", fontFamily: "'Barlow', sans-serif", fontSize: 13, fontWeight: er ? 700 : 500, color: er ? t.accent : WARM_GRAY, borderBottom: er ? "2px solid "+t.accent : "2px solid transparent", textAlign: "center", transition: "all 0.2s", marginBottom: -1, touchAction: "manipulation" }}>
              {t.label}
            </button>
          );
        })}
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "28px 24px 56px" }}>
        {aktivTab === "laere" && <OvelsesBrukerhistorie accent={LLP_LAERE} onFerdig={function(bh) { setBrukerhistorie(bh); }} onNeste={function(){ setAktivTab("lage"); }} />}
        {aktivTab === "lage" && <OvelsesCrazyEight accent={LLP_LAGE} brukerhistorie={brukerhistorie} onFerdig={function(vinner) { setVinnerIde(vinner); }} onNeste={function(){ setAktivTab("prove"); }} />}
        {aktivTab === "prove" && <OvelsesBrukercase accent={LLP_PROVE} vinnerIde={vinnerIde} onNeste={null} />}
      </div>
    </section>
  );
}
...
function OvelsesBrukerhistorie({ accent, onFerdig, onNeste }) {
  ...
  if (ferdig) return (
    <div>
      <p style={{ color: WARM_GRAY, fontSize: 11, letterSpacing: 1.5, textTransform: "uppercase", fontWeight: 700, marginBottom: 16 }}>Din brukerhistorie</p>
      ...
      <div style={{ marginTop: 16, display: "flex", gap: 10, flexWrap: "wrap" }}>
        <button onClick={function() { setRolle(""); setBehov(""); setVerdi(""); setFerdig(false); }} style={{ background: "none", color: accent, border: "1px solid "+accent, padding: "9px 20px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 600 }}>Prøv igjen</button>
        {onNeste && <button onClick={onNeste} style={{ background: accent, color: "white", border: "none", padding: "9px 20px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 700, touchAction: "manipulation" }}>Gå videre til Lage →</button>}
      </div>
    </div>
  );
  return (
    <div>
      <p style={{ fontSize: 13, color: WARM_GRAY, lineHeight: 1.65, marginBottom: 20 }}>En brukerhistorie holder fokus på hvem du lager noe for. Fyll inn feltene og se historien ta form.</p>
      <div style={{ background: "white", border: "1px solid rgba(0,0,0,0.06)", borderRadius: 12, padding: "20px", marginBottom: 20 }}>
        <p style={{ fontSize: 14, color: INK, lineHeight: 2.2, margin: 0 }}>
          Som{" "}<input value={rolle} onChange={function(e){setRolle(e.target.value);}} placeholder="hvem?" style={{ border: "none", borderBottom: "2px solid "+accent, outline: "none", fontSize: 16, color: INK, width: 120, fontFamily: "'Barlow', sans-serif", background: "transparent", padding: "0 4px" }} />
          {" "}vil jeg{" "}<input value={behov} onChange={function(e){setBehov(e.target.value);}} placeholder="hva?" style={{ border: "none", borderBottom: "2px solid "+accent, outline: "none", fontSize: 16, color: INK, width: 160, fontFamily: "'Barlow', sans-serif", background: "transparent", padding: "0 4px" }} />
          {" "}slik at{" "}<input value={verdi} onChange={function(e){setVerdi(e.target.value);}} placeholder="hvorfor?" style={{ border: "none", borderBottom: "2px solid "+accent, outline: "none", fontSize: 16, color: INK, width: 160, fontFamily: "'Barlow', sans-serif", background: "transparent", padding: "0 4px" }} />.
        </p>
      </div>
      <button onClick={function() { if (kanSende) { setFerdig(true); onFerdig && onFerdig({ rolle: rolle, behov: behov, verdi: verdi }); } }} disabled={!kanSende}
        style={{ background: kanSende ? accent : "#ddd", color: kanSende ? "white" : WARM_GRAY, border: "none", padding: "11px 24px", borderRadius: 8, fontSize: 14, cursor: kanSende ? "pointer" : "default", fontFamily: "'Barlow', sans-serif", fontWeight: 700 }}>
        Lag brukerhistorien →
      </button>
    </div>
  );
...
function OvelsesCrazyEight({ accent, brukerhistorie, onFerdig, onNeste }) {
  var [steg, setSteg] = useState("intro");
  ...
  function startTimer() { setSteg("aktiv"); setSekLeft(60); timerRef.current = setInterval(function() { setSekLeft(function(s) { if (s <= 1) { clearInterval(timerRef.current); setSteg("stem"); return 0; } return s - 1; }); }, 1000); }
  useEffect(function() { return function() { if (timerRef.current) clearInterval(timerRef.current); }; }, []);
  function oppdaterIdé(i, val) { setIdeer(function(prev) { var ny = prev.slice(); ny[i] = val; return ny; }); }
  function giStemme(i) { if (prikkerIgjen <= 0) return; setStemmer(function(prev) { var ny = prev.slice(); ny[i]++; return ny; }); setPrikkerIgjen(function(p) { return p - 1; }); }
  function reset() { setIdeer(["","","",""]); setStemmer([0,0,0,0]); setPrikkerIgjen(5); setSekLeft(60); setSteg("intro"); }
  var vinner = stemmer.indexOf(Math.max.apply(null, stemmer));

  if (steg === "intro") return (
    <div>
      {brukerhistorie && (
        <div style={{ background: LLP_LAERE+"15", border: "1px solid "+LLP_LAERE+"44", borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
          <p style={{ fontSize: 9, color: LLP_LAERE, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>Ramme fra Lære</p>
          <p style={{ fontSize: 13, color: INK, margin: 0, lineHeight: 1.6 }}>Som <strong>{brukerhistorie.rolle}</strong> vil jeg <strong>{brukerhistorie.behov}</strong> slik at <strong>{brukerhistorie.verdi}</strong>.</p>
        </div>
      )}
      <p style={{ fontSize: 13, color: WARM_GRAY, lineHeight: 1.65, marginBottom: 16 }}>Tving hjernen til å produsere idéer raskt. Du har 60 sekunder – ikke tenk, bare skriv.</p>
      <button onClick={startTimer} style={{ background: accent, color: "white", border: "none", padding: "13px 28px", borderRadius: 8, fontSize: 15, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 700, touchAction: "manipulation" }}>Start ▶</button>
    </div>
  );
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ width: 38, height: 38, borderRadius: "50%", border: "3px solid "+accent, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        {ideer.map(function(ide, i) {
          return (
            <div key={i} style={{ background: "white", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 8, padding: "10px 14px" }}>
              ...
              <textarea value={ide} onChange={function(e){oppdaterIdé(i,e.target.value);}} placeholder="Skriv raskt..." style={{ width: "100%", border: "none", outline: "none", fontSize: 16, fontFamily: "'Barlow', sans-serif", resize: "none", background: "transparent", color: INK, lineHeight: 1.5, display: "block" }} rows={1} />
            </div>
          );
        })}
      </div>
      <div style={{ marginTop: 14 }}>
        <button onClick={function(){ clearInterval(timerRef.current); setSteg("stem"); }} style={{ background: "none", color: accent, border: "1px solid "+accent, padding: "9px 20px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 600, touchAction: "manipulation" }}>Ferdig – stem nå →</button>
      </div>
    </div>
  );
  ...
  if (steg === "stem") return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>{[...Array(5)].map(function(_, i) { return <div key={i} style={{ width: 10, height: 10, borderRadius: "50%", background: i < prikkerIgjen ? accent : "rgba(0,0,0,0.1)" }} />; })}</div>
      </div>
      <p style={{ fontSize: 12, color: WARM_GRAY, marginBottom: 16 }}>Du har {prikkerIgjen} prikk{prikkerIgjen !== 1 ? "er" : ""} igjen.</p>
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
        {ideer.map(function(ide, i) {
          return (
            <div key={i} style={{ background: stemmer[i] > 0 ? accent+"15" : "white", border: "1px solid "+(stemmer[i] > 0 ? accent : "rgba(0,0,0,0.08)"), borderRadius: 8, padding: "12px 14px", cursor: "pointer", touchAction: "manipulation" }}
              onClick={function(){ prikkerIgjen > 0 ? giStemme(i) : null; }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ display: "flex", gap: 3, marginLeft: 12 }}>{[...Array(stemmer[i])].map(function(_, j) { return <div key={j} style={{ width: 8, height: 8, borderRadius: "50%", background: accent }} />; })}</div>
              </div>
            </div>
          );
        })}
      </div>
      <button onClick={function(){ setSteg("ferdig"); }} style={{ background: accent, color: "white", border: "none", padding: "11px 24px", borderRadius: 8, fontSize: 14, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 700, touchAction: "manipulation" }}>Se resultatet →</button>
    </div>
  );

  if (steg === "ferdig") return (
    <div>
      <p style={{ fontSize: 18, fontFamily: "'Caveat', cursive", color: accent, marginBottom: 14 }}>{ideer[vinner] ? "\""+ideer[vinner]+"\" vant!" : "Stemmeresultat"}</p>
        ...
        {ideer.map(function(ide, i) {
          ...
          return (
            <div key={i} style={{ background: erVinner ? accent+"20" : "white", border: "1px solid "+(erVinner ? accent : "rgba(0,0,0,0.06)"), borderRadius: 8, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div><p style={{ fontSize: 9, color: erVinner ? accent : WARM_GRAY, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 3 }}>Idé {i+1}{erVinner ? " 🏆" : ""}</p><p style={{ fontSize: 13, color: ide ? INK : WARM_GRAY, margin: 0, fontStyle: ide ? "normal" : "italic" }}>{ide || "tom"}</p></div>
              <div style={{ display: "flex", gap: 3, marginLeft: 12 }}>{[...Array(stemmer[i])].map(function(_, j) { return <div key={j} style={{ width: 8, height: 8, borderRadius: "50%", background: accent }} />; })}</div>
            </div>
          );
        })}
      ...
      <div style={{ marginTop: 14, display: "flex", gap: 10, flexWrap: "wrap" }}>
        <button onClick={reset} style={{ background: "none", color: accent, border: "1px solid "+accent, padding: "9px 20px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 600, touchAction: "manipulation" }}>Prøv igjen</button>
        {onNeste && <button onClick={function(){ onFerdig && onFerdig(ideer[vinner] || ""); onNeste(); }} style={{ background: accent, color: "white", border: "none", padding: "9px 20px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 700, touchAction: "manipulation" }}>Gå videre til Prøve →</button>}
      </div>
    </div>
  );
  return null;
...
function OvelsesBrukercase({ accent, vinnerIde }) {
  ...
  if (ferdig) return (
    <div>
      <div style={{ background: "white", border: "1px solid "+accent+"44", borderRadius: 10, padding: "18px", marginBottom: 16 }}>
        <div style={{ marginBottom: 12 }}><p style={{ fontSize: 10, color: accent, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>Brukeren</p><p style={{ fontSize: 14, color: INK, margin: 0 }}>{bruker}</p></div>
        <div style={{ marginBottom: 12 }}><p style={{ fontSize: 10, color: accent, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>Situasjon</p><p style={{ fontSize: 14, color: INK, margin: 0 }}>{situasjon}</p></div>
        <div style={{ paddingTop: 12, borderTop: "1px solid rgba(0,0,0,0.06)" }}><p style={{ fontSize: 10, color: accent, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>Oppgave</p><div style={{ background: accent+"12", borderRadius: 6, padding: "10px 14px" }}><p style={{ fontSize: 15, color: INK, margin: 0, fontWeight: 600 }}>{oppgave}</p></div></div>
      </div>
      <StickyNote text="Tre brukere er nok til å finne de største problemene." />
      <div style={{ marginTop: 14 }}><button onClick={function() { setBruker(""); setSituasjon(""); setOppgave(""); setFerdig(false); }} style={{ background: "none", color: accent, border: "1px solid "+accent, padding: "9px 20px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 600 }}>Lag ny testcase</button></div>
    </div>
  );
  return (
    <div>
      {vinnerIde && <div style={{ background: LLP_LAGE+"15", border: "1px solid "+LLP_LAGE+"44", borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}><p style={{ fontSize: 9, color: LLP_LAGE, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>Vinnende idé fra Lage</p><p style={{ fontSize: 13, color: INK, margin: 0, fontWeight: 600 }}>{vinnerIde}</p></div>}
      <p style={{ fontSize: 13, color: WARM_GRAY, lineHeight: 1.65, marginBottom: 20 }}>En testcase gir brukeren kontekst og en konkret oppgave å løse med prototypen din.</p>
      <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 20 }}>
        <div><p style={{ fontSize: 11, color: WARM_GRAY, fontWeight: 600, letterSpacing: 0.5, marginBottom: 6 }}>Hvem er brukeren?</p><input value={bruker} onChange={function(e){setBruker(e.target.value);}} placeholder="Eks: En saksbehandler som jobber med byggesøknader..." style={{ width: "100%", padding: "11px 14px", fontSize: 16, border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, outline: "none", fontFamily: "'Barlow', sans-serif", color: INK }} /></div>
        <div><p style={{ fontSize: 11, color: WARM_GRAY, fontWeight: 600, letterSpacing: 0.5, marginBottom: 6 }}>Hva er situasjonen?</p><textarea value={situasjon} onChange={function(e){setSituasjon(e.target.value);}} placeholder="Eks: Det er mandag morgen og hun har fått inn en ny søknad..." rows={3} style={{ width: "100%", padding: "11px 14px", fontSize: 16, border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, outline: "none", fontFamily: "'Barlow', sans-serif", color: INK, resize: "none", lineHeight: 1.55 }} /></div>
        <div><p style={{ fontSize: 11, color: WARM_GRAY, fontWeight: 600, letterSpacing: 0.5, marginBottom: 6 }}>Hva er oppgaven?</p><input value={oppgave} onChange={function(e){setOppgave(e.target.value);}} placeholder="Eks: Finn ut hva som mangler i søknaden og send tilbakemelding." style={{ width: "100%", padding: "11px 14px", fontSize: 16, border: "1px solid rgba(0,0,0,0.12)", borderRadius: 8, outline: "none", fontFamily: "'Barlow', sans-serif", color: INK }} /></div>
      </div>
      <button onClick={function(){if(kanSende)setFerdig(true);}} disabled={!kanSende} style={{ background: kanSende ? accent : "#ddd", color: kanSende ? "white" : WARM_GRAY, border: "none", padding: "11px 24px", borderRadius: 8, fontSize: 14, cursor: kanSende ? "pointer" : "default", fontFamily: "'Barlow', sans-serif", fontWeight: 700 }}>Generer testcase →</button>
    </div>
  );
}
...
function SeksjonKontakt({ answers, oppfolgingSvar }) {
  var rolle = answers[0] || "annet";
  var innhold = {
    tonsberg: { tittel: "Vi sitter ikke langt unna.", ingress: "Er du ansatt i Tønsberg kommune og vil prøve noe nytt i avdelingen din? Ta kontakt direkte.", badge: "Intern kontakt" },
    annenkommune: { tittel: "Vi deler gjerne erfaringer.", ingress: "Er du ansatt i en annen kommune og nysgjerrig på hvordan vi jobber? Vi er åpne for erfaringsutveksling.", badge: "Tverrkommunalt samarbeid" },
    forsker: { tittel: "Vi er interessert i samarbeid.", ingress: "Forsker du på innovasjon i offentlig sektor? Vi er åpne for akademisk samarbeid og datainnsamling.", badge: "Forskning og samarbeid" },
    annet: { tittel: "Book oss for en uforpliktende prat!", ingress: "Lurer du på hvordan TønsbergLØFTET kan hjelpe? Ta kontakt – vi er alltid nysgjerrige.", badge: null },
  };
  var t = innhold[rolle] || innhold.annet;
  return (
    <section style={{ background: NAVY_MID, padding: "64px 24px" }}>
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        {answers[2] && <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 14, marginBottom: 24 }}>Du forventet: <em>"{answers[2]}"</em>{oppfolgingSvar ? " – og svarte " : ""}<strong style={{ color: "rgba(255,255,255,0.75)" }}>{oppfolgingSvar || ""}</strong></p>}
        {t.badge && <div style={{ display: "inline-block", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 20, padding: "4px 12px", fontSize: 11, color: "rgba(255,255,255,0.55)", letterSpacing: 1, textTransform: "uppercase", fontWeight: 700, marginBottom: 16 }}>{t.badge}</div>}
        ...
        <a href="mailto:postmottak@tonsberg.kommune.no" style={{ background: YELLOW, color: INK, padding: "12px 24px", borderRadius: 8, fontSize: 15, textDecoration: "none", fontWeight: 700, display: "inline-block" }}>Send e-post</a>
      </div>
    </section>
  );
}
...
export default function App() {
  ...
  function handleChoice(id) { setPendingChoice(id); }
  ...
  return (
    <div>
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Barlow:wght@400;500;600;700;800&family=Caveat:wght@500;600;700&display=swap');
      ...
      {phase === "intro" && (
        <div style={{ minHeight: "100vh", background: OFF_WHITE, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", fontFamily: "'Barlow', sans-serif" }}>
          <button onClick={function() { setPhase("static"); }} style={{ position: "fixed", top: 16, right: 16, background: "none", border: "none", color: WARM_GRAY, fontSize: 12, cursor: "pointer", opacity: 0.5, fontFamily: "'Barlow', sans-serif" }}>Hopp over</button>
          <div style={{ maxWidth: 500, width: "100%", animation: "fadeIn 0.6s ease" }}>
            ...
            {introStep === 1 && (
              <div style={{ animation: "fadeIn 0.5s ease" }}>
                <p style={{ color: NAVY, fontWeight: 700, fontSize: 11, letterSpacing: 1, textTransform: "uppercase", marginBottom: 16 }}>Før du scroller videre</p>
                <h2 style={{ fontSize: "clamp(22px,6vw,30px)", fontWeight: 800, color: INK, marginBottom: 14, lineHeight: 1.2 }}>Du er brukeren vår. Vi er nysgjerrige.</h2>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <button onClick={function() { setPhase("survey"); }} style={{ background: NAVY, color: "white", border: "none", padding: "13px 24px", borderRadius: 8, fontSize: 15, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 700, textAlign: "left" }}>Svar på tre spørsmål →</button>
                  <button onClick={function() { setPhase("main"); }} style={{ background: "none", color: NAVY, border: "2px solid "+NAVY, padding: "12px 24px", borderRadius: 8, fontSize: 15, cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: 700, textAlign: "left" }}>Gå rett til siden</button>
                </div>
              </div>
            )}
          </div>
      ...
      {phase === "survey" && (
        <div style={{ minHeight: "100vh", background: NAVY_BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 24px", fontFamily: "'Barlow', sans-serif" }}>
          <button onClick={function() { setPhase("main"); }} style={{ position: "fixed", top: 16, right: 16, background: "rgba(255,255,255,0.08)", border: "none", color: "rgba(255,255,255,0.5)", fontSize: 12, cursor: "pointer", padding: "6px 12px", borderRadius: 20, fontFamily: "'Barlow', sans-serif" }}>Hopp over</button>
          <div style={{ maxWidth: 480, width: "100%" }}>
            <div style={{ display: "flex", gap: 6, marginBottom: 28 }}>
              {survey.map(function(_, i) { return <div key={i} style={{ height: 3, borderRadius: 2, flex: 1, background: i < qIndex ? "rgba(255,255,255,0.6)" : i === qIndex ? "white" : "rgba(255,255,255,0.15)", transition: "background 0.3s" }} />; })}
            </div>
            <div key={qIndex} style={{ animation: "fadeIn 0.35s ease" }}>
              <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, letterSpacing: 1.5, textTransform: "uppercase", fontWeight: 700, marginBottom: 8 }}>{q.fase} · {qIndex+1}/{survey.length}</p>
              <h2 style={{ fontSize: "clamp(22px,6vw,30px)", color: "white", fontWeight: 800, marginBottom: 8, lineHeight: 1.25 }}>{q.sporsmal}</h2>
                  ...
                  {q.valg.map(function(v) {
                    ...
                    return (
                      <button key={v.id} onClick={function() { handleChoice(v.id); }}
                        style={{ background: isSelected ? "white" : "rgba(255,255,255,0.07)", border: "1px solid "+(isSelected ? "white" : "rgba(255,255,255,0.15)"), borderRadius: 10, padding: "14px 18px", fontSize: 15, color: isSelected ? NAVY_BG : "rgba(255,255,255,0.8)", cursor: "pointer", fontFamily: "'Barlow', sans-serif", fontWeight: isSelected ? 700 : 400, textAlign: "left", transition: "all 0.2s" }}>
                        {v.label}
                      </button>
                    );
                  })}
              ...
              {q.type === "text" && (
                <textarea value={currentText} onChange={function(e) { setCurrentText(e.target.value); }} onKeyDown={function(e) { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleNext(); } }} placeholder="Skriv gjerne fritt..." rows={3} autoFocus
                  style={{ width: "100%", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.18)", borderRadius: 10, padding: 16, color: "white", fontSize: 16, fontFamily: "'Barlow', sans-serif", resize: "none", outline: "none", lineHeight: 1.6, display: "block" }} />
              )}
              <div style={{ marginTop: 20, display: "flex", justifyContent: "flex-end" }}>
                <button onClick={handleNext} disabled={!canProceed}
                  style={{ background: canProceed ? "white" : "rgba(255,255,255,0.15)", color: canProceed ? NAVY_BG : "rgba(255,255,255,0.3)", border: "none", padding: "12px 24px", borderRadius: 8, fontSize: 15, cursor: canProceed ? "pointer" : "default", fontFamily: "'Barlow', sans-serif", fontWeight: 700, transition: "all 0.2s" }}>
      ...
      {phase === "transition" && (
        <div style={{ minHeight: "100vh", background: NAVY_BG, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32, textAlign: "center", fontFamily: "'Barlow', sans-serif" }}>
          <div style={{ maxWidth: 440 }}>
            ...
            {transitionStep >= 2 && <p style={{ color: "white", fontSize: 18, fontWeight: 600, animation: "fadeIn 0.6s ease", lineHeight: 1.6 }}>Her er siden – satt opp for deg.</p>}
          </div>
        </div>
      )}
